      ...
      <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="col-md-4" style="padding-top:20px;">
                ...
                <br />
                <br/>
                <button class="btn btn-success crop_image" value="{{$dosen->id}}">Crop & Upload Image</button>
            </div>
          </div>
            </div>
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
      <div class="card">
        <div class="card-header">
          <h2>
              {{$dosen->namagelar}}
              <small>{{$dosen->user}}</small>
              <a class="btn btn-primary mx-1 float-right" href="{{ route('dosen.index') }}"><i class="fas fa-arrow-left"></i><span></span></a>
          </h2>
        </div>
        <div class="card-body">
          <div class="row clearfix">
            <div class="col-md-3">
              <img src="{{ url($dosen->userz->foto.'/display_foto')}}" id="preview" class="border w-100" onerror="this.onerror=null;this.src='{{asset('fileupload/a.jpg')}}';">
              ...
              <div class="form-group">
                <div class="col">
                  <h4>Barcode</h4>
                  <?php 
                    echo '<img src="data:image/png;base64,' . DNS1D::getBarcodePNG($dosen->user, 'C128') . '" alt="barcode"/>';
                    ?>
                  <br>
                  <a class="my-2 btn btn-secondary w-100" href="{{ url("admin/dosen/$dosen->id/idcard/show") }}">Lihat Id Card</a>
                </div>
              </div>
            </div>
            <div class="card col-md-9">
              <div class="card-body">
                @if ($biodata = $dosen->biodata)
                  <div class="row">
                    <div class="col-md-6">
                      <div class="row">
                        <div class="col-sm-5">
                ...
                @else
                  <div class="col-md-12">
                    <form method="POST" action="{{ route('dosen.biodata.create', $dosen->id) }}">
                      @csrf
                ...
                @endif
                  <div class="col-md-12">
                    <button type="button" class="btn btn-warning float-right m-1" data-toggle="modal" data-target="#updateBiodataModal"><i class="fas fa-edit"></i></button>
                  </div>
              </div>
          ...
          <div class="card-body">
              ...
              <div class="tab-content" id="nav-tabContent">
                <div class="tab-pane fade show active" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
                  <button type="button" class="btn btn-success my-3" onclick="location.href='{{ route('dosen.riwayat-golongan.create', $dosen->id ) }}'"><i class="fas fa-plus-circle"></i><span>Tambah Riwayat Golongan</span></button>
                    @if(!$dosen->riwayatgolongan->isEmpty())
                      <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                            ...
                            <tbody>
                            @foreach($dosen->riwayatgolongan as $i)
                                <tr>
                                    ...
                                    <td><a type="button" class="btn btn-warning" href="{{ route('dosen.riwayat-golongan.edit', ['dosen' => $dosen->id, 'riwayat_golongan' => $i->id] ) }}"><i class="far fa-edit"></a></td>
                                    <td>
                                        <form method="POST" action="{{route('dosen.riwayat-golongan.destroy', ['dosen' => $dosen->id, 'riwayat_golongan' => $i->id])}}">
                                            @csrf
                                            @method("DELETE")
                ...
                <div class="tab-pane fade" id="riwayat-jabatan" role="tabpanel" aria-labelledby="riwayat-jabatan-tab">
                  <button type="button" class="btn btn-success mt-3" onclick="location.href='{{ route('dosen.riwayat-jabatan.create', $dosen->id ) }}'"><i class="fas fa-plus-circle"></i><span> Tambah Riwayat Jabatan</span></button>
                  @if(!$dosen->riwayatjabatan->isEmpty())
                    <div class="table-responsive">
                      <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                          <thead>
                              <tr>
                          ...
                          <tbody>
                          @foreach($dosen->riwayatjabatan as $i)
                              <tr>
                                  <td>{{ $loop->iteration}}</td>
                                  ...
                                  <td>{{ $i->nama_ttd}}</td>
                                  <td>{{ $i->tmt->translatedFormat('d F Y')}}</td>
                                  <td><a type="button" class="btn btn-warning" href="{{ route('dosen.riwayat-jabatan.edit', ['dosen' => $dosen->id, 'riwayat_jabatan' => $i->id] ) }}"><i class="far fa-edit"></a></td>
                                  <td>
                                      <form method="POST" action="{{route('dosen.riwayat-jabatan.destroy', ['dosen' => $dosen->id, 'riwayat_jabatan' => $i->id])}}">
                                          @csrf
                                          @method("DELETE")
                                          <button type="submit" class="btn btn-danger">
                                          <i class="far fa-trash-alt"></i>
                ...
                <div class="tab-pane fade" id="nav-pendidikan" role="tabpanel" aria-labelledby="nav-pendidikan-tab">
                  <button type="button" class="btn btn-success my-3" onclick="location.href='{{  route('dosen.riwayat-pendidikan.create', $dosen->id) }}'"><i class="fas fa-plus-circle"></i><span>Tambah Riwayat Pendidikan</span></button>
                  @if(!$dosen->riwayatpendidikan->isEmpty())
                    <div class="table-responsive">
                      <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                          <thead>
                              <tr>
                          ...
                          <tbody>
                          @foreach($dosen->riwayatpendidikan as $i)
                              <tr>
                                  <td>{{ $loop->iteration}}</td>
                                  ...
                                  <td>{{ $i->namasekolah}}</td>
                                  <td>{{ $i->noijazah}}</td>
                                  <td><a type="button" class="btn btn-warning" href="{{ route('dosen.riwayat-pendidikan.edit', ['dosen' => $dosen->id, 'riwayat_pendidikan' => $i->id] ) }}"><i class="far fa-edit"></a></td>
                                  <td>
                                      <form method="POST" action="{{route('dosen.riwayat-pendidikan.destroy', ['dosen' => $dosen->id, 'riwayat_pendidikan' => $i->id])}}">
                                          @csrf
                                          @method("DELETE")
                                          <button type="submit" class="btn btn-danger">
                                          <i class="far fa-trash-alt"></i>
                ...
                <div class="tab-pane fade" id="nav-dokumen" role="tabpanel" aria-labelledby="nav-dokumen-tab">
                  <button type="button" class="btn btn-success my-3" onclick="location.href='{{ route('dosen.dokumen.create', $dosen->id) }}'"><i class="fas fa-plus-circle"></i><span>Tambah Dokumen</span></button>
                  @if(!$dosen->dokumen->isEmpty())
                      <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                            <thead>
                                <tr>
                            ...
                            @foreach($dosen->dokumen as $i)
                                <tr>
                                    ...
                                    <td><button type="button" class="btn btn-primary" onClick="window.open('{{ route('dosen.dokumen.show', ['dosen' => $dosen->id, 'dokuman' => $i->id]) }}');"><i class="fas fa-eye"></i></button></td>
                                    <td>
                                      <form method="POST" action="{{route('dosen.dokumen.destroy', ['dosen' => $dosen->id, 'dokuman' => $i->id])}}">
                                          @csrf
                                          @method("DELETE")
                                          <button type="submit" class="btn btn-danger">
                                          <i class="far fa-trash-alt"></i>
      ...
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <form method="POST" action="{{ route('dosen.biodata.update', $biodata->id) }}">
            <div class="modal-body">
              @csrf
              @method("put")
                  <div class="form-row">
                    <div class="form-group col">
                      ...
                      <div class="form-group">
                          <div class="form-line">
                              <input type="text" name="nik" class="form-control" value="{{ $biodata->nomorktp ?? "" }}">
                            </div>
                  ...
                  <div class="form-row">
                    <div class="form-group col">
                      ...
                      <div class="form-group">
                          <div class="form-line">
                              <input type="text" name="tempatlahir" class="form-control" value="{{ $biodata->tempatlahir ?? "" }}">
                    ...
                    <div class="form-group col">
                      ...
                      <div class="form-group">
                          <div class="form-line" id="bs_datepicker_container">
                              <input type="date" name="tanggallahir" class="form-control" value="{{$biodata->tanggallahir}}">
                  ...
                  <div class="form-row">
                    ...
                    <div class="form-group col-md-3"id="kota" >
                        ...
                        <select name="kota" class="form-control " id="id_kota" data-live-search="true">
                          <option value="{{$biodata->Kota->id ?? ""}}">{{$biodata->Kota->name ?? "Pilih Kota/Kabupaten"}}</option>
                        </select>
                    ...
                    <div class="form-group col-md-3" id="distrik">
                      ...
                      <select name="kecamatan" class="form-control "  id="id_distrik">
                          <option value="{{$biodata->Kecamatan->id ?? ""}}">{{$biodata->Kecamatan->name ?? "Pilih Kecamatan"}}</option>
                        </select>
                    ...
                    <div class="form-group col-md-3">
                      ...
                      <select name="kelurahan" class="form-control show-tick"  id="id_desa">
                          <option value="{{$biodata->Kelurahan->id ?? ""}}">{{$biodata->Kelurahan->name ?? "Pilih Kelurahan/Desa"}}</option>
                        </select>
  ...
  $(document).ready(function () {
        ...
        $('#id_kota').change(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    $distrik.html('<option value="" selected>Pilih Kecamatan</option>');
                    $.each(data, function (id, value) {
        ...
        $('#id_distrik').change(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    $desa.html('<option value="" selected>Pilih Kelurahan/Desa</option>');
                    $.each(data, function (id, value) {
