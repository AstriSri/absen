      ...
      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
          <div class="card">
            <div class="card-header">
              ...
              <div class="card-body">
                <form method="POST" action="{{ route('riwayat-golongan.update', $riwayatgolongan->id) }}">
                  @csrf
                  @method("PUT")
                    ...
                    <div class="form-group">
                      <select name="golongan" class="form-control show-tick" data-live-search="true">
                        ...
                        @foreach($golongan as $b)
                            <option value="{{ $b->id}}" {{ $riwayatgolongan->golongan == $b->id ? "selected":"" }}>{{ $b->golongan}}</option>
                        @endforeach
                      </select>
                    ...
                    <button type="button" class="btn btn-danger m-1 float-right" data-toggle="modal" data-target="#deleteModal" data-action="{{route('riwayat-golongan.destroy', $riwayatgolongan->id)}}">Delete</button>
                </form>
              </div>
          </div>
        </div>
